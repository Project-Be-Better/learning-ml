...
Since mlflow has dependency conflicts with Python 3.13, this module provides
a drop-in compatible tracking interface that writes experiment runs as JSON
manifests to the `mlruns/` directory — the same filesystem structure MLFlow uses.

Usage is identical to mlflow:
    from src.core.experiment_tracker import tracker
    with tracker.start_run(experiment="my-experiment") as run:
        tracker.log_param("n_estimators", 100)
        tracker.log_metric("r2_score", 0.97)
        tracker.set_tag("quality_gate", "PASSED")
        tracker.log_artifact("models/model.joblib")
"""
